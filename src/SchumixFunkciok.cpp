...
void IRCSession::SvnArcemu(int rev, string channel)
{
	...
	snprintf(url, 400, "http://arcemu.info/trac/arcemu/changeset/%i", rev);
	string urlcim = url;
	delete[] url;
	...
	if(commit == "" && author == "")
		...
		SendChatMessage(PRIVMSG, channel.c_str(), "3Arcemu rev 10%i. Fejlesztõ: 12%s. Commit:%s", rev, author.c_str(), commit.c_str());
}

void IRCSession::SvnArcScripts(int rev, string channel)
{
	char* url = new char[400];
	snprintf(url, 400, "http://arcemu.info/trac/arcscripts/changeset/%i", rev);
	string urlcim = url;
	delete[] url;
	...
	if(commit == "" && author == "")
		...
		SendChatMessage(PRIVMSG, channel.c_str(), "3ArcScripts rev 10%i. Fejlesztõ: 12%s. Commit:%s", rev, author.c_str(), commit.c_str());
}
...
void IRCSession::SvnAscNHalf(int rev, string channel)
{
	char* url = new char[400];
	snprintf(url, 400, "http://www.assembla.com/code/ascnhalf/subversion/changesets/%i", rev);
	string urlcim = url;
	delete[] url;
	...
	if(m_Curl)
	{
		...
		if(result == CURLE_OK)
		{
			boost::regex re("<p class=.commit-description.>\\s*<pre>(.*)</pre>\\s*</p>");
			boost::cmatch matches;
	...
	if(m_Curl)
	{
		...
		if(result == CURLE_OK)
		{
			boost::regex re("<p class=\\\"committer-info\\\"><span>Author:</span> <a href=\\\"/profile/(.*)\\\" tabindex=");
			boost::cmatch matches;
			...
			if(matched == "")
			{
				boost::regex re1("<p class=\\\"committer-info\\\"><span>Author:</span> (.*)</p>\\s*<p class=\\\"commit-date\\\">");
	...
	if(commit == "" && author == "")
		...
		SendChatMessage(PRIVMSG, channel.c_str(), "3AscNHalf rev 10%i. Fejlesztõ: 12%s. Commit:%s", rev, author.c_str(), commit.c_str());
}

void IRCSession::SvnSchumix(int rev, string channel)
{
	char* url = new char[400];
	snprintf(url, 400, "http://www.assembla.com/code/schumix/subversion/changesets/%i", rev);
	string urlcim = url;
	delete[] url;
	...
	if(commit == "" && author == "")
		...
		SendChatMessage(PRIVMSG, channel.c_str(), "3Schumix rev 10%i. Fejlesztõ: 12%s. Commit:%s", rev, author.c_str(), commit.c_str());
}
