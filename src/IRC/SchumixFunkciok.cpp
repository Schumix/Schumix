...
void IRCSession::Schumix(IRCMessage& recvData)
{
	...
	if(recvData.Args.substr(0, vesszo) == _nick)
	{
		...
		else if(iras == "sys")
		{
			...
			SendChatMessage(PRIVMSG, recvData.GetChannel(), "3Memória használat: %f MB", ((sVezerlo.MemoryInfo(GetCurrentProcessId())/1024)/1024));
#else
			SendChatMessage(PRIVMSG, recvData.GetChannel(), "3Memória használat: %f MB", ((sVezerlo.MemoryInfo()/1000)*0.3762));
#endif
			SendChatMessage(PRIVMSG, recvData.GetChannel(), "3Működési idő: %s", sVezerlo.GetUptimeString().c_str());
		}
		else if(iras == "help")
